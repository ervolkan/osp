...
**Corpus.** 15 open-source repositories selected for diversity across language (Python 9 repositories, TypeScript 3 repositories, JavaScript 3 repositories), maturity (small libraries to large frameworks), and workflow (merge-commit, squash, rebase, solo). One Python repository is a solo-author baseline. Repositories were cloned with full Git history. The analyzer also supports Rust and Go adapters (tree-sitter tier), but no Rust/Go repositories are included in the reported corpus.
