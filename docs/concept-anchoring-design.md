...
- `docs/invariant-spec.md` — INV #1-15 (Paper 1) + INV-T1-T8 (Paper 2); INV-C1-C7 bu dokümanda proposal, implementation'da Bölüm C'ye taşınır
...
2. **Concept Synthesis + Genesis Layer genişletmesi** — Project Genesis Layer kavramı, PositionFamily, lane modeli, SupersedeAuthority, INV-C1-C7 önerileri
...
pub enum PositionFamily {
    ...
    Evidence,           // test/metric kanıt konumları
}
```
...
Eğer ikisini aynı Inferred sepetine koyarsak "coupling 0.82" ile "LLM bence bu modül ödemevizyonu" aynı epistemik seviyeye iner — bu OSP'nin ontolojik duruşunu zedeler. Kod gerçektir; koddan çıkarılan *anlam* yorumdur.
...
0.40 <= score < 0.60  → CreateNode veya WeaklyAnchored
...
|---|---|---|
| **High** | DerivesRule, DerivesTask, Contradicts, Supersedes, ExpectedImplementation, ImplementedBy, EvidencedBy, AntiGoalOf, Constrains | **zorunlu** (skor kırılımı + reason) |
...
## 9. INV-C1..C7 Invariant Önerileri (Proposal)

Aşağıdaki 7 invariant Paper 3'ün omurgasıdır. **Durum: proposal** — implementation sırasında (`crates/osp-core/src/anchoring/` oluşunca) `docs/invariant-spec.md` Bölüm C'ye taşınır. INV #1-15 (Paper 1) ve INV-T1-T8 (Paper 2) ile çakışmadıkları doğrulanmalıdır.
...
**Yapısal garanti:** `PositionSnapshot.family: PositionFamily` alanı; `PositionVector` family'ye göre farklı eksen seti taşır.
...
**İhlal örneği:** "PaymentModule coupling 0.82" (Observed metric) ile "PaymentModule ödemevizyonu yansıtıyor" (Inferred yorum) aynı epistemik seviyede Accepted status'a konur → Paper 1 ontolojisi çöker.
...
**Karar:** High-stake anchor decision'lar (DerivesRule, DerivesTask, Contradicts, Supersedes, ExpectedImplementation) operator onayı gerektirir. Düşük-stake (Mentions, RelatedTo) serbest. Ayrıca yüksek abstraction → doğrudan kod bağlantısı yasak (ara node şart).
...
**Karar:** INV-C6 düzeltildi — kod metric'leri Observed, koddan çıkarılan niyet Inferred.

**Gerekçe (itiraz 2):** Kodun ölçülen yapısı gerçektir (Paper 1); koddan çıkarılan anlam yorumdur. İkisini aynı Inferred sepetine koymak Paper 1 ontolojisini çökertir.

**OSP tutarlılık:** Paper 1 + Paper 3 ontolojik uyum.
...
**Karar:** INV-C7 kapsamı daraltıldı — high-stake edge'ler (9 tür) için explanation zorunlu; düşük-stake (4 tür) opsiyonel.
...
Faz 2 — Scoring + decision
  ...
  INV-C1-C7 enforcement (type-level)
...
**Sonraki adım:** Bu doküman review ile v0.2'ye ilerler, sonra Faz 0 fixture'ları (10 golden cümle) hazırlanır, ardından Faz 1 in-memory MVP başlar.

---

*Doküman kaynakları: özgün `concept-anchoring-algorithm-requirements.md` (v0.1-draft) + Concept Synthesis/Genesis Layer genişletmesi + 5 sınır hassasiyeti düzeltmesi. INV-C1-C7 implementation'da `docs/invariant-spec.md` Bölüm C'ye taşınır.*
